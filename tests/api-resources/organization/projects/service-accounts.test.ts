...
describe('resource serviceAccounts', () => {
  ...
  test.skip('delete', async () => {
    const responsePromise = client.organization.projects.serviceAccounts.delete(
      'project_id',
      'service_account_id',
    );
    const rawResponse = await responsePromise.asResponse();
  ...
  test.skip('delete: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.organization.projects.serviceAccounts.delete('project_id', 'service_account_id', {
        path: '/_stainless_unknown_path',
      }),
    ).rejects.toThrow(ExCai.NotFoundError);
  });

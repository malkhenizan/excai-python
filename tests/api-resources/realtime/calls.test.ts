...
describe('resource calls', () => {
  ...
  // Prism tests are disabled
  test.skip('hangup: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.realtime.calls.hangup('call_id', { path: '/_stainless_unknown_path' }),
  ...
  test.skip('reject: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.realtime.calls.reject('call_id', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(ExCai.NotFoundError);
  });
